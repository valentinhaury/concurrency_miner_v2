...
##### acitivity once per trace
    ...
    if len(activities_once_per_trace_partitions) >  1:
        return create_children_from_sublogs(Node(Operator.Concurrent), create_sublogs_general(log, activities_once_per_trace_partitions), concurrency_miner, filter_threshold)
...
##### activity concurrent
    ...
    if len(activity_concurrent_partitions) > 1:
        return create_children_from_sublogs(Node(Operator.Concurrent), create_sublogs_general(log, activity_concurrent_partitions), concurrency_miner, filter_threshold)
