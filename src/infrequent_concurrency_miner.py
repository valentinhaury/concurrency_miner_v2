...
##### initiate log parameters
    log_activities, log_follows = set(), set()
    counter_overlapping, counter_directly_follows, counter_start_activities, counter_end_activities = Counter(), Counter(), Counter(), Counter()
    ...
    for trace in log:
        ...
        counter_start_activities.update(trace.get_start_activities())
        #log_end_activities |= trace.get_end_activities()                # contains all end activities
        counter_end_activities.update(trace.get_end_activities())
    ...
    def filter_activities(counter, threshold):
        ...
        for a, count in counter.items():
            if count / max_count >= threshold:
                filtered_activities.add(a)
        return filtered_activities

##### filter start and end activities
    filtered_start_activities = filter_activities(counter_start_activities, filter_threshold)
    filtered_end_activities = filter_activities(counter_end_activities, filter_threshold)
