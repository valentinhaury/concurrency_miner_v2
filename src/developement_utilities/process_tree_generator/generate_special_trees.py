from src.data_structures.event import Event
from src.data_structures.trace import Trace
from src.data_structures.process_tree import Node
from src.data_structures.process_tree_operator import Operator


def generate_tree_1():
    tree = Node(Operator.Arbitrary)
    child1 = Node("A")
    child2 = Node(Operator.Exclusive)

    child2_1 = Node(Operator.Multi)
    child2_1_1 = Node("B")

    child2_2 = Node("C")

    child2_1.add_child(child2_1_1)
    child2.add_child(child2_1)
    child2.add_child(child2_2)
    tree.add_child(child1)
    tree.add_child(child2)
    return tree

def generate_tree_example_1():
    tree = Node(Operator.Arbitrary)

    child1 = Node("A")
    tree.add_child(child1)

    child2_1 = Node(Operator.Sequence)
    child2_1_1 = Node(Operator.Interleaving)
    child2_1_1_1 = Node("C")
    child2_1_1_2 = Node("D")
    child2_1_1.add_child(child2_1_1_1)
    child2_1_1.add_child(child2_1_1_2)

    child2_1_2 = Node("E")
    child2_1.add_child(child2_1_1)
    child2_1.add_child(child2_1_2)

    child2_2 = Node("B")

    child2 = Node(Operator.Parallel)
    child2.add_child(child2_1)
    child2.add_child(child2_2)
    tree.add_child(child2)

    return tree

# WIRD NICHT SO GEFUNDEN -> Arbitrary wird fälschlicherweise in interleafed integriert
def generate_tree_3():
    tree = Node(Operator.Arbitrary)
    child1 = Node(Operator.Arbitrary)
    child2 = Node(Operator.Exclusive)
    child1_1 = Node("A")
    child1_2 = Node("B")

    child1.add_child(child1_1)
    child1.add_child(child1_2)

    child2_1 = Node("C")
    child2_2 = Node("D")

    child2.add_child(child2_1)
    child2.add_child(child2_2)
    tree.add_child(child1)
    tree.add_child(child2)
    return tree

# nicht gefunden loop(e, sequence(multi(a), loop(c, d, b)))
def generate_tree_4():
    child3_1_1 = Node("B")
    child3_1 = Node(Operator.Multi)
    child3_1.add_child(child3_1_1)

    child3_2_1 = Node("C")
    child3_2_2 = Node("D")
    child3_2_3 = Node("E")
    child3_2 = Node(Operator.Loop)
    child3_2.add_child(child3_2_1)
    child3_2.add_child(child3_2_2)
    child3_2.add_child(child3_2_3)

    child1 = Node("A")
    child3 = Node(Operator.Sequence)
    child3.add_child(child3_1)
    child3.add_child(child3_2)

    tree = Node(Operator.Loop)
    tree.add_child(child1)
    tree.add_child(child3)
    return tree

def generate_test_tree_test_log_for_precision():
    a = Event("A")
    b = Event("C")
    spo = {(a, b)}
    trspo = {(a, b)}
    ovl = set()
    t1 = Trace({a, b}, trspo, spo, ovl)

    test_log = [t1]

    test_tree = Node(Operator.Concurrent)

    child1 = Node("A")
    child2 = Node(Operator.Loop)
    child3 = Node("tau")
    child4 = Node("C")
    child5 = Node("B")
    child2.add_child(child3)
    child2.add_child(child4)
    child2.add_child(child5)
    test_tree.add_child(child1)
    test_tree.add_child(child2)

    return test_log, test_tree