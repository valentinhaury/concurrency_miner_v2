...
def compute_sequence_sublogs(log, partitions):
    sublogs = []
    for partition in partitions:
        sub_log = []
        for trace in log:
            new_trace = ConcurrencyGroup()
            new_trace.events = partition & trace.events
            new_trace.follows = set()
            for relation in trace.follows:
                if set(relation).issubset(new_trace.events):
                    new_trace.follows.add(relation)
            new_trace.concurrency_pairs = set()
            for relation in trace.concurrency_pairs:
                if set(relation).issubset(new_trace.events):
                    new_trace.concurrency_pairs.add(relation)
            new_trace.directly_follows = set()
            for relation in trace.directly_follows:
                if set(relation).issubset(new_trace.events):
                    new_trace.directly_follows.add(relation)
            new_trace.start_activities = partition & trace.start_activities
            new_trace.end_activities = partition & trace.end_activities
